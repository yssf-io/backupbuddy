...
def recover_from_shares():
    try:
        ...
        for i in range(threshold):
            ...
            print(f"[DEBUG] Input Type: {type(raw)}")
            print(f"[DEBUG] Raw Value: {raw}")
            share_words = normalize_share_input(raw)
            entered_shares.append(share_words)

        print(f"[DEBUG] Entered shares: {entered_shares}")
        print(f"[DEBUG] Entered shares type: {type(entered_shares)} | Elements: {[type(s) for s in entered_shares]}")
