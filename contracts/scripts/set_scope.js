...
async function main() {
  ...
  const deploymentInfo = JSON.parse(
    fs.readFileSync("./deployments/latest.json", "utf8")
  );
  ...
  const newScope = process.env.GENERATED_SCOPE;

  if (!newScope) {
    console.error("Please provide the new scope value:");
    console.error("Set NEW_SCOPE environment variable:");
    console.error(
      "  NEW_SCOPE=<scope_value> npx hardhat run scripts/set_scope.js --network <network>"
    );
    console.error("");
    console.error("Example:");
    console.error(
      "NEW_SCOPE=1234567890 npx hardhat run scripts/set_scope.js --network alfajores"
    );
    process.exit(1);
  }
